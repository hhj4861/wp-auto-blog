#!/usr/bin/env python3
"""쿠팡 제휴 전용 신규 글 발행/재포맷 (trendpulse, CI 실행).

trendpulse 기존 글 포맷을 그대로 따른다 (미네랄 글 #827 실측 기준):
- 래퍼: <div class="post-content category-건강" data-category="건강">
- 히어로: figure.hero-image (Unsplash, 캡션 크레딧) + featured_media
- H2: 그린-틸 그라디언트 텍스트
- 본문 p: max-width 800 / line-height 1.8 / #cbd5e1
- 광고 2유닛 + 함께 보면 좋은 글 박스 (insert_monetization)
- 🛒 쿠팡 버튼 + 의무 고지문 유지
멱등: 동일 슬러그가 있으면 그 글을 같은 포맷으로 업데이트한다.
"""
...
from src.post_format import faq_section, sources_section  # noqa: E402

FAQ = [
    ("Q1. 멜라토닌은 매일 먹어도 되나요?",
     "국내 유통 식물성 멜라토닌은 저용량(1일 2mg 수준)이면 단기 사용은 무난하지만, 장기·고용량은 의존이 아니라 <strong style=\"color:#6ee7b7\">수면 위생과 병행</strong>하는 게 원칙입니다. 임신·수유·자가면역·항응고제 복용 시 복용 전 의사 상담이 필요합니다."),
    ("Q2. 테아닌이랑 멜라토닌 같이 먹어도 되나요?",
     "네, 작용 지점이 달라 함께 쓰는 배합 제품이 많습니다. 멜라토닌은 <strong style=\"color:#60a5fa\">수면 리듬</strong>, 테아닌은 <strong style=\"color:#60a5fa\">각성 완화</strong>를 담당해 입면 지연형·과각성형에 각각 맞습니다."),
    ("Q3. 유산균이 수면과 무슨 상관인가요?",
     "직접적인 수면 성분은 아니지만 장-뇌 축 연구가 쌓이며 장 환경이 스트레스 호르몬·수면의 질에 영향을 준다는 근거가 늘고 있어요. 기초 체력처럼 깔아두는 성분입니다."),
    ("Q4. 2주 먹어도 효과가 없어요.",
     "코골이·무호흡이 의심되거나 2주 이상 개선이 없으면 영양제가 아니라 <strong style=\"color:#60a5fa\">수면클리닉 진료</strong>가 맞습니다. 오후 카페인부터 끊는 게 순서입니다."),
]
SOURCES = [
    "(식품의약품안전처) 건강기능식품 기능성 원료(멜라토닌·테아닌·프로바이오틱스) 정보",
    "(질병관리청 국가건강정보포털) 불면증·수면 위생",
    "(대한수면학회) 수면 위생 일반 정보",
]
...
{P}선택 기준은 세 가지입니다: ① 식약처 인증 여부 ② 멜라토닌 함량이 명확히 표기돼 있을 것 ③ 테아닌 등 보조 성분의 배합량 공개. 이 조건을 채운 제품 중 접근성이 좋은 것이 아래 제품입니다.</p>
...
def fetch_hero(api: str, auth: tuple) -> tuple:
    """Unsplash 수면 이미지 → WP 업로드. (media_id, figure_html) 반환. 실패 시 (None, "")."""
    ...
    try:
        r = requests.get(
            ...
            params={"query": "sleeping night bedroom rest", "per_page": 1,
        ...
        alt = photo.get("alt_description") or "peaceful sleep at night"
        credit = photo["user"]["name"]

        img = requests.get(img_url, timeout=30)
        img.raise_for_status()
        headers = {"Content-Disposition": 'attachment; filename="sleep-supplement-hero.jpg"',
