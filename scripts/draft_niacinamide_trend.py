#!/usr/bin/env python3
"""나이아신아마이드 가이드 (trendpulse, 한글) — [[스캘폴드: 본문 TODO 채우기]].
검색모듈 블로그용 선정 키워드(월검색량 95,400). 캐논 포맷·올리브영·쿠팡고지·네이버 export 배선 완료.
"""
import os
import re
import sys
...
SLUG = "niacinamide-guide-2026"
TITLE = "[[TODO 제목]] — 나이아신아마이드 (2026)"
META_DESC = "[[TODO 메타설명 150자 내외]]"
FOCUS_KW = "나이아신아마이드"
OLIVE = ("https://global.oliveyoung.com/display/search?query="
         + quote("나이아신아마이드") + "&rwardCode=HHJZ4861&utm_source=influencers")
...
ARTICLE = f"""
{P}[[TODO 인트로: 나이아신아마이드 가 뭐고 왜 관심인지]]</p>
...
{H2}어떻게 고르나</h2>
{P}[[TODO 고르는 기준]]</p>
{product_button("나이아신아마이드", COUPANG) if COUPANG else ""}
{olive_link("나이아신아마이드")}
...
def fetch_hero(api, auth):
    ...
    try:
        ...
        up = requests.post(f"{api}/media", auth=auth,
                           headers={"Content-Disposition": 'attachment; filename="niacinamide-guide-2026.jpg"',
