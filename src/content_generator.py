...
4. **내부 링크 (필수 1개 이상)**:
   ...
   - 카테고리별 링크 (반드시 영문 slug 사용!):
     - 취업: <a href="https://trendpulse.blog/category/jobs/">취업 가이드</a>
     - 테크: <a href="https://trendpulse.blog/category/tech/">테크 트렌드</a>
     - 건강: <a href="https://trendpulse.blog/category/wellness/">건강 정보</a>
     - 리뷰: <a href="https://trendpulse.blog/category/review/">리뷰 모음</a>
     - 생산성: <a href="https://trendpulse.blog/category/productivity/">생산성 가이드</a>
     - 비즈니스: <a href="https://trendpulse.blog/category/biz/">비즈니스 인사이트</a>
   - 예: "외항사 취업에 관심 있다면 <a href=\"https://trendpulse.blog/category/jobs/\">취업 가이드</a>도 참고하세요."
    ...
    def generate(
        ...
        if market_brief is not None:
            ...
            planned_scope = {
                ...
                'current_relevance': {key: suitability_plan['current_relevance'][key]
            ...
            prompt += (
                ...
                'required_facets의 모든 필수 질문에 답하고, sources에 검증된 기관·대상·비용 범위를 유지하세요. '
                'current_relevance의 현재 일정과 과거 일정을 구분하세요. 한 기관의 추가검사만으로 전체 비용 안내를 대신하지 마세요. '
                '기획의 예정 날짜도 새 공식 원문으로 다시 확인하고 달라졌거나 확인할 수 없으면 기존 날짜를 사실로 쓰지 마세요. '
                '본문 사실은 새로 읽은 공식 원문으로만 뒷받침하세요. 검색량/점수는 본문에 넣지 마세요.\n'
                + json.dumps({**{key: market_brief[key] for key in ('keyword', 'intent', 'gap')},
                              **planned_scope}, ensure_ascii=False))
            research_data = ''
        else:
...
DO NOT use Markdown. Use only HTML tags."""
        ...
        html = self._fix_category_links(html)
...
Be specific and factual based on search results. Always use the most recent version numbers."""
    ...
    def _fix_category_links(self, html: str) -> str:
        """Fix Korean category links to use English slugs.

        LLM often generates category links with Korean names or URL-encoded Korean,
        but WordPress uses English slugs. This converts them to the correct format.

        Args:
            html: HTML content with potentially Korean category links

        Returns:
            HTML with corrected English slug category links
        """
        # TrendPulse 한글 카테고리 -> 영문 slug 매핑
        category_slug_map = {
            # 한글 그대로
            "건강": "wellness",
            "리뷰": "review",
            ...
            "테크": "tech",
            # URL-encoded 한글
            "%EA%B1%B4%EA%B0%95": "wellness",      # 건강
            "%EB%A6%AC%EB%B7%B0": "review",        # 리뷰
            "%EB%B9%84%EC%A6%88%EB%8B%88%EC%8A%A4": "biz",  # 비즈니스
            "%EC%83%9D%EC%82%B0%EC%84%B1": "productivity",  # 생산성
            "%EC%B7%A8%EC%97%85": "jobs",          # 취업
            "%ED%85%8C%ED%81%AC": "tech",          # 테크
        }

        result = html
        fix_count = 0

        for korean, english in category_slug_map.items():
            # Pattern: trendpulse.blog/category/한글/ or trendpulse.blog/category/한글"
            old_pattern = f"trendpulse.blog/category/{korean}"
            new_pattern = f"trendpulse.blog/category/{english}"
        ...
        if fix_count > 0:
            logger.info(f"Fixed {fix_count} Korean category links to English slugs")
