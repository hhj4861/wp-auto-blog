    ...
    try:
        if args.refresh_slugs:
            ...
            def _pick_next():
                ...
                시즌 우선순위(코드로 강제, 프롬프트 아님):
                  - publish_by가 미래인 시즌 토픽은 아직 선택하지 않는다(색인 리드타임 확보)
                  - publish_by가 도래한 시즌 토픽은 검색량과 무관하게 최우선(마감 임박 순)
                  - 그 외에는 검색량 큰 순 (큐 파일 순서)
                """
                cands = [
                ...
                def key(item):
                    pb = item.get("publish_by")
                    if pb:
                        try:
                            pb_date = _dt.date.fromisoformat(pb)
                        except ValueError:
                            pb_date = None
                        if pb_date and pb_date > today:
                            # 아직 발행 시기 아님 — 맨 뒤로
                            return (2, 0)
                        # 발행 시기 도래 — 최우선(마감 임박=publish_by 이른 순)
                        return (0, pb or "")
                    return (1, -(item.get("monthly_search") or 0))

                cands.sort(key=key)
                top = cands[0]
                # 전부 '아직 시기 아님'이면(=시즌 토픽만 남음) None 취급하지 않고
                # 그래도 하나는 발행 (빈 슬롯 방지) — 단 로그로 알린다
                if top.get("publish_by"):
                    try:
                        if _dt.date.fromisoformat(top["publish_by"]) > today:
                            logger.info(
                                f"시즌 토픽만 남음 — 리드타임 전이지만 발행: {top['topic']}")
                    except ValueError:
                        pass
                return top
            ...
            while skips < 5:
                ...
                if result.success and not args.dry_run:
                    ...
                    if result.post:
                        ...
                        pending_topic["post_status"] = result.post.status.value
                    timestamp_key = "held_at" if pending_topic["status"] == "held_draft" else "completed_at"
                    pending_topic[timestamp_key] = _dt.datetime.now().isoformat()
                    _save_queue()
                    logger.info(f"Queue updated: marked as {pending_topic['status']}")
                break
