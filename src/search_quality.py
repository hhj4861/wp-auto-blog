...
def review_search(keyword, provider, results, checked_at, call_llm, *, executed_query=None):
    ...
    if rows:
        prompt = (
            ...
            'relevant=true는 검색어의 핵심 대상과 정보 요구에 직접 답하거나 그 답변에 필요한 '
            '구체적인 구성 요소를 제공하는 결과에 허용합니다. 개별 검색 결과 하나가 검색어 전체를 '
            '완결되게 설명해야 하는 것은 아닙니다. 검색 관련성과 최종 글의 출처 충족 범위는 별도 검수입니다. '
            '예를 들어 CT 비용의 흉부·복부별 실제 가격, MRI 비용의 부위별 가격표, '
            '심장 초음파 비용의 경흉부 심초음파 일반·단순 검사 가격은 해당 비용 질문의 구성 요소입니다. '
            '다만 가격표의 검사명만 있고 가격이나 비용 안내가 제목·발췌문에 없으면 관련성을 추정하지 마세요. '
            '갑상선에 좋은 음식에서 저하증·항진증별 식사 설명도 관련 결과입니다. '
            '같은 단어 일부만 겹치는 회사정보·주가·공연장·다른 검사/제품은 false입니다. '
            'CT 비용에서 PET-CT만의 가격, 심장 초음파 비용에서 갑상선 검사 가격, '
