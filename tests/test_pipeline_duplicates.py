...
def test_all_general_categories_create_one_draft_before_waiting_for_affiliate_reply(
    ...
    content.html = '<h2>' + keyword + '</h2><p>' + keyword + '는 공식 준비물과 신청 절차를 확인하세요.</p>'
    content.keywords = [keyword]
    content.focus_keyphrase = keyword
    content.meta_description = keyword + '의 공식 준비물과 절차를 안내합니다.'
    ...
    if not quality_passes:
        monkeypatch.setattr(pipeline_module, 'check_quality', lambda **kw: ['본문 검수 실패'])
    pipeline.wp_client.create_post.side_effect = lambda **kw: CreatedPost(
        1749, 'https://trendpulse.blog/approval-draft/', content.title, kw['status'])
    monkeypatch.setattr(entry, '__file__', str(tmp_path / 'src' / 'main.py'))
    monkeypatch.setattr(entry, 'BlogPipeline', lambda *a, **kw: pipeline)
    monkeypatch.setattr(entry, 'load_dotenv', lambda: None)
    monkeypatch.setattr(entry, 'setup_logging', lambda **kw: None)
    monkeypatch.setenv('BLOG_COUPANG_TELEGRAM', '1')
    monkeypatch.setenv('BLOG_RESULT_PATH', str(result_path))
    monkeypatch.setattr(sys, 'argv', ['blog', '--mode', 'general', '--from-queue',
...
def test_affiliate_dry_run_preserves_pending_queue_and_never_registers_draft_zero(
    ...
    pipeline = market_pipeline
    pipeline.config.dry_run = True
    pipeline._is_duplicate = Mock(return_value=False)
    brief = candidate()
    path = tmp_path / 'data' / 'topic_queue_general.json'
    path.parent.mkdir(exist_ok=True)
    original = json.dumps([brief], ensure_ascii=False)
    path.write_text(original)
    result_path = tmp_path / 'result.json'
    monkeypatch.setattr(market, 'LEDGER', path.parent / 'posted_market_keywords.json')
    recorder = Mock(side_effect=AssertionError('dry run must not record a publication'))
    monkeypatch.setattr(market, 'record_published_keyword', recorder)
    monkeypatch.setattr(entry, '__file__', str(tmp_path / 'src' / 'main.py'))
    monkeypatch.setattr(entry, 'BlogPipeline', lambda *a, **kw: pipeline)
    monkeypatch.setattr(entry, 'load_dotenv', lambda: None)
    monkeypatch.setattr(entry, 'setup_logging', lambda **kw: None)
    ...
    monkeypatch.setenv('BLOG_RESULT_PATH', str(result_path))
    monkeypatch.setattr(sys, 'argv', ['blog', '--mode', 'general', '--from-queue',
                                    '--category', '취업', '--auto-publish', '--dry-run'])
    assert entry.main() == 0
    assert path.read_text() == original
    pipeline.wp_client.create_post.assert_not_called()
    recorder.assert_not_called()
    result = json.loads(result_path.read_text())[0]
    assert result['post_id'] == 0 and result['awaiting_affiliate'] is False
    assert not market.LEDGER.exists()
